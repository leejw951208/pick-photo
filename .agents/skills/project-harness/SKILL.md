...
Write `.agents/**` skill files and `AGENTS.md` in English. Write human-facing documents outside `.agents/` in Korean, including PRDs, feature docs, implementation plans, feature progress tracking, review docs, change summaries, and commit messages. Format commit messages as `<commit type>: <Korean message>` on `main` or `dev`; use `[<branch>]<commit type>: <Korean message>` on any other branch.
...
8. Ask the user to confirm the approved plan before implementation.
